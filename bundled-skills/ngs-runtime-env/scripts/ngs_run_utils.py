#!/usr/bin/env python3
"""Shared helpers for skill-bundled NGS execution runners."""
...
def _find_skill_root() -> Path | None:
    for current in [Path(__file__).resolve().parent, *Path(__file__).resolve().parents]:
        if (current / "SOURCE.json").exists() and (current / "SKILL.md").exists():
            return current
    return None


def skill_bundle_metadata() -> dict[str, Any]:
...
def write_standard_manifest(
    ...
    *,
    ...
    merged_audit = {
        "skill_bundle": skill_bundle_metadata(),
