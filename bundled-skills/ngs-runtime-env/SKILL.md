...
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --tool fastqc --emit-install-plan
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --profile local_light --emit-install-plan
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline dna_variant_calling --network-checks --emit-install-plan
...
Use `inventory` when the user needs a broader resource/database audit across the skill bundle. It writes `resource_inventory.json`, `resource_inventory.tsv`, `resource_env.sh`, and `resource_dashboard.md`, including missing files, env vars, setup hints, license notes, and pipeline usage for every known bundle.
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_nfcore_pipeline.py --pipeline taxprofiler --sample-sheet samples.csv --profile docker --bundle-root kraken2_standard=/db/kraken2/standard --include-optional-resources
```
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline fastq_qc --manager mamba --install-missing --yes
```
