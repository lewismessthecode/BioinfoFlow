...
- `ngs-fastq-qc`: skill-bundled local runner for FASTQ validation, FastQC/MultiQC execution, optional trimming, logs, summaries, and artifact indexes.
- `ngs-bulk-rnaseq-counts-qc`: skill-bundled local runner for bulk RNA-seq FASTQ validation, FastQC/MultiQC, Salmon transcript quantification, TPM/NumReads/effective-length matrices, logs, summaries, and artifact indexes.
- `ngs-bulk-rnaseq-differential-expression`: skill-bundled local runner for count-matrix validation, contrast/replicate checks, automatic DESeq2/edgeR/limma method selection, QC plots, normalized matrices, result tables, logs, summaries, and artifact indexes.
- `ngs-scrna-seq`: skill-bundled local FASTQ-to-count runner for STARsolo-backed scRNA/snRNA count generation.
...
- `ngs-dna-variant-calling`: skill-bundled BAM/CRAM-to-VCF execution package using samtools/bcftools for focused local checks, with nf-core/sarek still preferred for full WGS/WES/panel workflows.
- `ngs-dna-germline-variants`: skill-bundled higher-fidelity germline runner for BQSR, per-sample gVCFs, and optional joint genotyping when a local GATK toolchain and matched known-sites resources are available.
- `ngs-epigenomics-peaks`: skill-bundled FASTQ validation/QC execution package for ATAC-seq, ChIP-seq, CUT&RUN, and CUT&Tag intake, with readiness artifacts for the alignment and peak-calling stage.
- `ngs-amplicon-microbiome`: skill-bundled FASTQ validation/QC execution package for marker-gene amplicon inputs, with explicit primer/taxonomy backend readiness artifacts.
- `ngs-shotgun-metagenomics`: skill-bundled FASTQ validation/QC execution package for shotgun metagenomics inputs, with explicit database-gated taxonomic profiling status.
- `ngs-bcl-to-fastq`: skill-bundled BCL run-folder and sample-sheet validator that executes BCL Convert or legacy bcl2fastq when an installed converter is available.
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --list
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline bulk_rnaseq --emit-install-plan
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline bulk_rnaseq_counts_qc --emit-install-plan
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline bulk_rnaseq_differential_expression --emit-install-plan
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --profile local_light --emit-install-plan
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --tool fastqc --network-checks
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline shotgun_metagenomics --manager micromamba --install-plan-outdir runtime_readiness/shotgun_install
```
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_fastq_qc.py \
...
The bulk RNA-seq counts/QC lane accepts an nf-core-style sample sheet, validates FASTQ paths and read structure, runs FastQC/MultiQC plus Salmon transcript quantification through a skill-bundled Snakemake workflow, and writes the standard run envelope.

```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_bulk_rnaseq_counts_qc.py \
  --sample-sheet samplesheet.csv \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_bulk_rnaseq_de.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_scrnaseq_post_count_qc.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_dna_variant_calling.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_dna_germline_variants.py \
  --sample-sheet dna_samples.tsv \
  --reference-fasta reference.fa \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_dna_somatic_variants.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_dna_umi_panel_variants.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_fastq_assay_package.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_atacseq_peaks_qc.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_chip_cutrun_peaks_qc.py \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_amplicon_microbiome.py \
  --sample-sheet amplicon_samples.tsv \
  --backend qiime2 \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_shotgun_metagenomics.py \
  --sample-sheet shotgun_samples.csv \
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py list
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py check --kind reference --bundle grch38_core --root /refs/GRCh38
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py explain-missing --kind database --bundle kraken2_standard --root /db/kraken2/standard
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py plan --pipeline shotgun_metagenomics --include-optional --outdir resource_readiness/shotgun
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py setup-plan --pipeline shotgun_metagenomics --include-optional --outdir resource_readiness/shotgun_setup
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py plan --pipeline atacseq --genome-build GRCh38 --bundle-root grch38_core=/refs/GRCh38 --outdir resource_readiness/atac
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py inventory --outdir resource_readiness/inventory
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py lock --outdir resource_readiness/lock --include-checksums
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py verify-lock --lockfile resource_readiness/lock/resource_lock.json --outdir resource_readiness/lock_verify --fail-on-mismatch
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py check-all --kind database --output resource_readiness/database_audit.json
```
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_bcl_to_fastq.py \
...
Do not distribute only the `$BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/` directory unless the recipient already knows how to register a Codex marketplace entry for it.
