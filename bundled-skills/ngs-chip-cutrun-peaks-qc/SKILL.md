...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline chip_cutrun_peaks_qc --emit-install-plan
```
...
For nf-core execution, use `$BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_nfcore_pipeline.py --pipeline chipseq` or `--pipeline cutandrun`.
