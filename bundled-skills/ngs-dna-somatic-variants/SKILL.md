...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline dna_somatic_variants --emit-install-plan
```
...
This produces a tumor-normal/tumor-only pairing table, Mutect2 command plan, contamination/filtering artifacts, somatic QC summary, `qc/somatic_pair_review.{tsv,json}`, visualization index, and filtered VCF outputs when the local GATK resources are available. For nf-core execution, use `$BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/run_nfcore_pipeline.py --pipeline sarek`.
