...
Use this skill for QC-only, trimming-first, or FASTQ quality interpretation workflows. This skill can execute the skill-bundled local FastQ QC runner when the user approves a local run. It should decide whether trimming or additional investigation is warranted; it should not blindly trim by default.
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline fastq_qc --emit-install-plan
```

## Local Execution

Use the skill-bundled runner for local artifact-producing FASTQ QC:
