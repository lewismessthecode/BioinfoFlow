...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline <pipeline_key> --emit-install-plan
```

When the user needs an approval-ready install handoff, write persistent install artifacts:

```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_preflight.py --pipeline <pipeline_key> --manager micromamba --install-plan-outdir runtime_readiness/<pipeline_key>_install
```
...
```bash
python $BIOINFOFLOW_SKILLS_ROOT/ngs-runtime-env/scripts/ngs_reference_manager.py plan --pipeline <pipeline_key> --genome-build <build> --outdir resource_readiness/<pipeline_key>
```
