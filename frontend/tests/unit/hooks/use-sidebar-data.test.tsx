...
describe("useSidebarData", () => {
  ...
  it("returns to the draft route when selecting a project from an existing agent session", async () => {
    const projects: Project[] = [
      { id: "project-a", name: "Alpha", project_root: "asset://project", storage_mode: "managed" },
      { id: "project-b", name: "Bravo", project_root: "asset://project", storage_mode: "managed" },
    ]
    apiRequestMock.mockImplementation(async (path) => {
      if (path === "/projects") return { data: projects, meta: undefined }
      if (path === "/projects/default") throw new Error("no default")
      if (path === "/agent/sessions") return { data: [], meta: undefined }
      throw new Error(`Unexpected path: ${path}`)
    })
    pathnameMock.mockReturnValue("/agent/session-old")
    ...
    await waitFor(() => expect(result.current.sidebar.projects).toHaveLength(2))
    const projectB = result.current.sidebar.projects.find(({ id }) => id === "project-b")
    expect(projectB).toBeDefined()

    act(() => {
      result.current.sidebar.handleSelectProject(projectB!)
    })

    expect(result.current.project.selectedProjectId).toBe("project-b")
    expect(result.current.project.activeConversationId).toBe("")
    expect(replaceMock).toHaveBeenCalledWith("/agent")
  })

  it("does not promote an inbox route to a workspace project", async () => {
    ...
    apiRequestMock.mockImplementation(async (path) => {
      if (path === "/projects") {
        return { data: [defaultProject, regularProject], meta: undefined }
      }
      if (path === "/projects/default") {
      ...
      if (path === "/agent/sessions") {
        return {
          data: [session({ id: "inbox-session", project_id: null })],
          meta: undefined,
        }
      }
      throw new Error(`Unexpected path: ${path}`)
    })
    pathnameMock.mockReturnValue("/agent/inbox-session")
    ...
    expect(replaceMock).not.toHaveBeenCalledWith("/agent")
  })
