...
The deterministic terminal screenshot fixture remains in place; terminal
production wiring is covered separately by the existing unit/integration
tests listed below.
...
was restored. No terminal production implementation was changed.
...
- `tests/integration/components/terminal-dock.test.tsx`: 9 tests covering
  TerminalDock session/header/error/theme/fit behavior with a deterministic
  `MockTerminal`.
- `tests/integration/components/app-layout-terminal.test.tsx`: 7 tests
...
- `rtk bun run test -- tests/unit/components/workspace-code-preview.test.tsx tests/integration/components/live-deck-wiring.test.tsx tests/integration/components/terminal-dock.test.tsx tests/integration/pages/agent-page.test.tsx tests/integration/components/app-layout-terminal.test.tsx`
  — 5 files, 36 tests.
- `rtk bun run test` — 186 files, 1025 tests.
