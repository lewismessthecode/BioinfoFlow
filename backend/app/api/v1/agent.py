...
from app.services.agent_ui.run_settings import freeze_message_run_settings
...
def _artifact_data(artifact) -> dict:
    ...
    return {
        ...
        "resource_ref": public_artifact_resource(artifact.resource_ref),
...
async def dispatch_command(
    ...
    async with _session_mutation_lock(session_id):
        ...
        if isinstance(command, MessageCommand):
            run_settings_snapshot = await freeze_message_run_settings(
                db,
                workspace_id=user.workspace_id,
                user_id=user.id,
