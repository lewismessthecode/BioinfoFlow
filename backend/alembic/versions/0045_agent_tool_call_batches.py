...
def upgrade() -> None:
    if (
        not _table_exists("agent_tool_call_batches")
        and _table_exists("agent_sessions")
        and _table_exists("agent_turns")
    ):
        op.create_table(
            ...
            sa.Column(
                "status", sa.String(30), nullable=False, server_default="evaluating"
            ),
            sa.Column("tool_call_count", sa.Integer(), nullable=False),
            sa.Column(
                "continuation_claimed_at", sa.DateTime(timezone=True), nullable=True
            ),
            ...
            sa.ForeignKeyConstraint(
                ["turn_id"], ["agent_turns.id"], ondelete="CASCADE"
            ),
            sa.PrimaryKeyConstraint("id"),
        )
        op.create_index(
            "ix_agent_tool_call_batches_session_id",
            "agent_tool_call_batches",
            ["session_id"],
        )
        op.create_index(
            "ix_agent_tool_call_batches_turn_id", "agent_tool_call_batches", ["turn_id"]
        )
        op.create_index(
            "ix_agent_tool_call_batches_status", "agent_tool_call_batches", ["status"]
        )

    if _table_exists("agent_actions") and _table_exists("agent_tool_call_batches"):
        columns = _column_names("agent_actions")
        with op.batch_alter_table("agent_actions") as batch:
            if "tool_batch_id" not in columns:
                batch.add_column(
                    sa.Column("tool_batch_id", sa.String(36), nullable=True)
                )
            ...
            if "tool_call_ordinal" not in columns:
                batch.add_column(
                    sa.Column("tool_call_ordinal", sa.Integer(), nullable=True)
                )
        with op.batch_alter_table("agent_actions") as batch:
        ...
        with op.batch_alter_table("agent_actions") as batch:
            if "tool_call_ordinal" in columns:
                batch.drop_constraint(
                    "uq_agent_actions_tool_batch_ordinal", type_="unique"
                )
                batch.drop_column("tool_call_ordinal")
            if "tool_batch_id" in columns:
                batch.drop_constraint(
                    "uq_agent_actions_tool_batch_call_id", type_="unique"
                )
                batch.drop_index("ix_agent_actions_tool_batch_id")
                batch.drop_constraint(
                    "fk_agent_actions_tool_batch_id", type_="foreignkey"
                )
