...
class TestConfigSet:
    def test_set_valid_key(self, runner: CliRunner, _patched_store) -> None:
        ...
        result = runner.invoke(
            app, ["config", "set", "base_url", "http://example.com/api/v1"]
        )
        assert result.exit_code == 0
        assert "base_url" in result.stdout

    def test_set_mode_key_is_removed(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
    ...
    def test_set_invalid_key(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        result = runner.invoke(
            app, ["config", "set", "invalid_key", "val"]
        )
    ...
    def test_set_json_output(self, runner: CliRunner, _patched_store) -> None:
        ...
        result = runner.invoke(
            app,
            ["--output", "json", "config", "set", "output", "json"],
        )
        assert result.exit_code == 0
        parsed = json.loads(result.stdout)
...
class TestConfigGet:
    def test_get_unset_key(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        result = runner.invoke(app, ["config", "get", "project_id"])
        assert result.exit_code == 0
    ...
    def test_get_set_key(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        runner.invoke(app, ["config", "set", "base_url", "http://example.com/api/v1"])
        result = runner.invoke(app, ["config", "get", "base_url"])
        assert result.exit_code == 0
        assert "http://example.com/api/v1" in result.stdout

    def test_get_mode_key_is_removed(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        result = runner.invoke(app, ["config", "get", "mode"])
        assert result.exit_code == 2
        assert "Unknown key" in result.stdout
    ...
    def test_get_json(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        result = runner.invoke(
            app, ["--output", "json", "config", "get", "base_url"]
        )
        assert result.exit_code == 0
        parsed = json.loads(result.stdout)
        assert parsed["success"] is True
...
class TestConfigShow:
    ...
    def test_show_json(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        runner.invoke(
            app, ["config", "set", "base_url", "http://example.com"]
        )
        result = runner.invoke(
            app, ["--output", "json", "config", "show"]
        )
        assert result.exit_code == 0
        parsed = json.loads(result.stdout)
        assert parsed["success"] is True
...
class TestConfigSetValidation:
    def test_set_mode_key_is_invalid(
        self, runner: CliRunner, _patched_store
    ) -> None:
        runner.invoke(app, ["config", "init"])
        result = runner.invoke(
            app, ["config", "set", "mode", "magic"]
        )
        assert result.exit_code == 2
...
class TestConfigUnset:
    def test_unset_existing(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        runner.invoke(app, ["config", "set", "project_id", "p-1"])
        result = runner.invoke(app, ["config", "unset", "project_id"])
        assert result.exit_code == 0
...
class TestConfigUseProject:
    def test_use_project(self, runner: CliRunner, _patched_store) -> None:
        runner.invoke(app, ["config", "init"])
        result = runner.invoke(
            app, ["config", "use-project", "proj-123"]
        )
        assert result.exit_code == 0
